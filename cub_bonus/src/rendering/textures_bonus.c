...
/*   Updated: 2025/02/05 23:02:13 by oel-feng         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	start_textures(t_map_data *map_data)
{
	...
	if (!map_data->textures[NORTH] || !map_data->textures[SOUTH]
		...
		ft_err(INVALID_TEXTURE);
	i = -1;
	while (++i < 5)
	{
		if (map_data->textures[i]->height != 64
			&& map_data->textures[i]->width != 64)
			ft_err(INVALID_TEXTURE_SIZE);
	}
